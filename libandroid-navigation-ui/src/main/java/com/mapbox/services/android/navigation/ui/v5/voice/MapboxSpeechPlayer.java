    ...
    voiceInstructionLoader.requestInstruction(instruction, textType, new Callback<ResponseBody>() {
      ...
      public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
        if (response.isSuccessful()) {
          ...
          try {
            Timber.e("DEBUG onResponse errorBody " + response.errorBody().string() + " request "
              + call.request().url().toString() + " code " + response.code());
            onError(response.errorBody().string());
          } catch (IOException exception) {
            Timber.e("DEBUG onResponse exception " + exception.getLocalizedMessage() + " request "
              + call.request().url().toString() + " code " + response.code());
      ...
      public void onFailure(Call<ResponseBody> call, Throwable throwable) {
        Timber.e("DEBUG onFailure " + throwable.getLocalizedMessage() + " request "
